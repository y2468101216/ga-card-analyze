...
function makeBuckets(items, getter) {
  ...
  const out = Array.from(map.entries())
    ...
    .sort((a, b) => {
      // numeric buckets first, then X, then Unknown
      const ax = a.bucket
      const bx = b.bucket
...
function estimateDiscard(raw) {
  const s = String(raw || '').toLowerCase()
  // MVP parser:
  // - "discard a card" / "discard one card" => 1
  // - "discard 2 cards" => 2
  // - "discard two cards" => 2
...
function analyze(deckText) {
  ...
  try {
    ...
    const material = hydrate(sections.material)
    const main = hydrate(sections.main)
    ...
    // draw/discard estimation + classification
    ...
    // Lists for image display: include imageUrl
...
app.post('/api/analyze', (req, res) => {
  try {
    res.json(analyze(req.body?.deckText))
  } catch (e) {
